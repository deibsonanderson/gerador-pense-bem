...
public class PenseBem {
	...
	private static final Integer TOTAL_LIVROS = 99; // maximo que livros é 99
	...
	public static void main(String[] args) {
		// readSample(precalc());
		precalc();
		Map<Integer, Map<Integer, List<Character>>> getMappedAnswers = getMappedAnswers(answers);
		var teste = getBook(getMappedAnswers, 89);
		System.out.println(toJson(teste));
	...
	// Esse metodo é responsavel por montar o mapa com as respostas no formato
	// MAP<Integer, Map<Integer, List<Character>>>
	public static Map<Integer, Map<Integer, List<Character>>> getMappedAnswers(
			ArrayList<ArrayList<Character>> answersParam) {
		Map<Integer, Map<Integer, List<Character>>> mapResult = new HashMap<>();

		for (int book = 0; book < answersParam.size(); book++) {
			Map<Integer, List<Character>> sessionMap = new HashMap<>();
			List<Character> bookAnswers = answersParam.get(book);
			for (int question = 0; question < bookAnswers.size(); question++) {
				sessionMap.computeIfAbsent((question / 30) + 1, session -> new ArrayList<>())
						.add(bookAnswers.get(question));
			}
			mapResult.put(book + 1, sessionMap);
		}

		return mapResult;
	}

	// Metodo que receba um inteiro e retorno um livro especifico com base no 0-99
	public static Map<Integer, Map<Integer, List<Character>>> getBook(Map<Integer, Map<Integer, List<Character>>> map,
			int book) {
		Map<Integer, Map<Integer, List<Character>>> mapResult = new HashMap<>();
		mapResult.put(book, map.get(book));
		return mapResult;
	}

	// Esse metodo é responsavel por converter o mapa em JSON
	public static String toJson(Map<Integer, Map<Integer, List<Character>>> map) {
		return "{\n" + map.entrySet().stream().map(e1 -> "  \"" + e1.getKey() + "\": {\n" +
				e1.getValue().entrySet().stream().map(e2 -> "    \"" + e2.getKey() + "\": [" +
						e2.getValue().stream().map(c -> "\"" + c + "\"").collect(Collectors.joining(", ")) +
